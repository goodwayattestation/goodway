...
function Showcase(props) {
    ...
    let lastPart = content.slice(content.length - (content.length % itemsPerRow), content.length);

    return (
        <div className={`organism-showcase xs-mt-6 xs-mb-6 md-mt-12 md-mb-12`}>
            <div className="container-fluid">
                {content.map((a, i) => (
                    <React.Fragment key={content[0].headline}>
                        {i % itemsPerRow === 0 && content.length - i >= itemsPerRow && (
                            <div className="row center-xs">
                                {[i, i + 1, i + 2, i + 3, i + 4].map((j, k) =>
                                    k < itemsPerRow ? (
                                        <div className="col-xs-12 col-sm-6 col-md-3" key={content[j].url}>
                                            <Card
                                                img={content[j].img}
                                                url={content[j].url}
                ...
                <div className="row center-xs">
                    {lastPart.map((c, i) => (
                        <div className="col-xs-12 col-sm-6 col-md-3" key={c.url}>
                            <Card
                                img={c.img}
                                url={c.url}
                                headline={c.headline}
