	...
	useEffect(() => {
		...
		document.addEventListener('resize', function () {
			...
			if (window.innerWidth < 1024) {
				...
				$(`.${styles['organism-navigation-bar_logo']}`).removeClass(
					styles['stickyLogoInNav']
				);
			}
		});
			...
			if (window.innerWidth > 1023) {
				...
				if (navbar) {
					...
					if (window.pageYOffset > sticky) {
						...
						$(`.${styles['organism-navigation-bar_logo']}`).addClass(
							styles['stickyLogoInNav']
						);

						document.querySelector(
							`.${styles['secondRow']} .${styles['otherservices']} + li`
						).style.display = 'none';
					} else {
						navbar.classList.remove(styles['stickyNav']);
						$(`.${styles['hideWhenSticky']}`).show();
		...
		<section className={styles['organism-navigation-bar']}>
			<div className={`container-fluid xs-mb-6`}>
				<div className="row middle-xs xs-pt-6 xs-pb-6">
					...
					<div
						className={`col-md-${
							props.showTrackStatusBtn ? '4 col-md-offset-1' : '6'
						} hidden-sm-down`}
					>
								...
								>
									{content.firstRow.contact.email.icon && (
										<FontAwesomeIcon
											...
											icon={
												content.firstRow.contact.email.icon === '3'
													? faEnvelope
													: ''
											}
										/>
					...
					<div
						className={`col-xs-7 col-md-${
							props.showTrackStatusBtn ? '4' : '3'
						} end-xs`}
					>
						<a
							className={`${
								props.showTrackStatusBtn ? 'xs-mr-12 md-mr-18' : ''
							} color-teal f-w:600 ${styles['directions']}`}
						...
						>
							{content.firstRow.directions.icon && (
								<FontAwesomeIcon
									...
									icon={
										content.firstRow.directions.icon === '4'
											? faLocationArrow
											: ''
									}
								/>
							...
							<span className="hidden-sm-down">
								{content.firstRow.directions.text}
							</span>
						</a>
				...
				<div
					className={`${styles['secondRow']} row middle-xs xs-pt-6 xs-pb-6 center-xs`}
				>
					<div className="col-xs-10 start-xs hidden-md-up">
						<div className="row start-xs">
							{content.firstRow.contact.numbers.map((c, i) => (
								<div
									className={`col-xs-12 col-sm-4 ${i === 0 ? 'xs-mb-6' : ''}`}
									key={i}
								>
									<a
