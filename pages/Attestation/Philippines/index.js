...
function PhilippinesAttestation(props) {
    useEffect(() => {
        document.title =
            'Attestation Service for Philippines in Bangalore - Philippines Certificate Attestation Near Me';
        document
            .querySelector('meta[name="description"]')
            .setAttribute(
                'content',
                'Philippines Certificate Attestation Service in Bangalore for Birth & Marriage Certificate, Education & Commercial Documents. For Quick Service Call:9148889666'
            );
        document
            .querySelector('meta[name="keywords"]')
            .setAttribute(
                'content',
                'Philippines Embassy Certificate Attestation Services, Certificate Attestation Service for Philippines, Philippines Embassy Attestation, Philippines Embassy Certificate Attestation in Bangalore, Philippines Embassy Document Attestation services'
            );
        document
            .querySelector('link[rel="canonical"]')
            .setAttribute('href', 'https://www.goodwayattestation.com/philippines-attestation');

        document.querySelector('#commonPageHeader').innerHTML = 'Attestation Service for Philippines in Bangalore';
    }, []);

    return (
        <div className="routes-philippines-attestation">
            <NavigationBar showTrackStatusBtn={true} />
            <AttestationTemplate headline="Attestation Service For Philippines In Bengaluru" tabs={Content}>
                <div className="routes-philippines-attestation_content">
                    <div className="row center-xs xs-mt-12 md-mt-54">
                        <div className="col-xs-12">
                            <h2 className="color-black">Certificates Required For Philippines Embassy Attestation</h2>
                        </div>
                    </div>
                    <div className="row color-black center-xs xs-mt-36 md-mt-48">
                        ...
                        <div className="col-xs-12 col-md-6 educationalCertificateAttestation">
                            ...
                            <p className="t-a:j xs-mt-18 md-mt-24 md-ml-48 f-w:500">
                                We also take over Ministry of Foreign Affairs of Philippines (MOFA of Philippines)*
                                After Attesting from country from which the documents are issued it has to be Attested
                                from MOFA Ministry of Foreign Affairs of the Country where you are going for employment.
                            </p>
                        </div>
                    </div>
                </div>
